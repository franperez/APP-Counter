...
// Inicializar eventos globales para detectar inputs matemáticos
document.addEventListener('focusin', function(e) {
    // Verificamos si el elemento enfocado tiene la clase requerida
    if (e.target && e.target.classList.contains('math-input')) {
        // Prevenir teclado nativo forzosamente
        e.target.setAttribute('inputmode', 'none');
        e.target.setAttribute('readonly', 'readonly'); // Truco para evitar parpadeo en algunos móviles
        
        // Quitar readonly momentáneamente para permitir el foco, luego volver a controlar
        setTimeout(() => {
...
// Cerrar teclado si se toca fuera del teclado y fuera de un input
document.addEventListener('click', function(e) {
    const numpad = document.getElementById('custom-numpad');
    const isInput = e.target.classList.contains('math-input');
    const isNumpad = numpad.contains(e.target);
    
    // Si el teclado está abierto y el clic no fue ni en el teclado ni en un input
    if (!numpad.classList.contains('d-none') && !isInput && !isNumpad) {
        closeNumpad();
    }
});
...
function openNumpad() {
    ...
    if(activeMathInput) {
        setTimeout(() => {
            activeMathInput.scrollIntoView({behavior: 'smooth', block: 'center'});
        }, 300);
    }
}
...
function numpadPress(key) {
    ...
    switch (key) {
        ...
        default:
            // --- LÓGICA CORREGIDA PARA DECIMALES ---
            if (key === '.') {
                // Separamos la cadena por los operadores (+, -, *, /)
                // Esto nos da un array de los números que componen la operación
                const segments = currentValue.split(/[+\-*/]/);
                // Obtenemos el último segmento (el número que se está escribiendo ahora)
                const lastNumber = segments[segments.length - 1];

                // Si el último número ya tiene un punto, no permitimos otro
                if (lastNumber.includes('.')) return;
                
                // Si el usuario empieza con un punto (ej: "* .5"), añadimos un 0 delante
                if (lastNumber === "") {
                    activeMathInput.value += "0";
                }
            }

            // Evitar múltiples operadores seguidos (ej: 5++5)
            const operators = ['+', '-', '*', '/'];
            if (operators.includes(key)) {
                const lastChar = currentValue.slice(-1);
                if (operators.includes(lastChar)) {
                    // Reemplazamos el operador anterior por el nuevo
                    activeMathInput.value = currentValue.slice(0, -1) + key;
                    return;
                }
            }

            // Insertar el número o punto
            if (currentValue === "0" && !operators.includes(key) && key !== '.') {
